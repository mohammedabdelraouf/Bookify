...
namespace backend.Data
{
    public class BookifyDbContext: IdentityDbContext<ApplicationUser>
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            // Storing the enum as string in the database
            builder.Entity<Room>()
                .Property(r => r.status).HasConversion<string>();

            builder.Entity<Booking>()
                .Property(b => b.bookingStatus).HasConversion<string>();
            ...
            builder.Entity<Booking>()
                .HasOne(b => b.Payment)
                .WithOne(p => p.Booking)
                .HasForeignKey<Payment>(p => p.BookingId); // <-- هنا إحنا حسمنا الجدل

            // booking - review one to one
            builder.Entity<Booking>()
                .HasOne(b=>b.Review).WithOne(r=>r.Book)
