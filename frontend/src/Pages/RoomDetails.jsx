...
const RoomDetails = () => {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (contentType && contentType.includes('application/json')) {
            ...
            errorMessage = error.message || error.error || 'Booking failed';
          } else {
            const text = await response.text();
            // Extract error message if it's in the text
            ...
            } else if (text.includes('Could not create booking')) {
              const match = text.match(/Could not create booking: ([^<"]+)/);
              errorMessage = match ? match[1].trim() : 'Room not available for selected dates';
            } else {
              errorMessage = response.status === 400 ? 'Invalid booking request' : 'Booking failed';
            }
          }
        } catch (parseError) {
      ...
      showToast(error.message || 'Unable to complete booking. Please try again.', 'error');
    } finally {
      setIsBooking(false);
    }
  };
